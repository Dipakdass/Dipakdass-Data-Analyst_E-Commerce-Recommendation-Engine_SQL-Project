...
-- 9. Run the inner query separately to verify if it returns the expected counts of interactions per product:

SELECT product_id, COUNT(*) AS interaction_count 
FROM Interactions 
GROUP BY product_id;

-- To remove safe coding
SET SQL_SAFE_UPDATES = 0;

-- 10. Update product price for a specific product:
UPDATE Products SET price = 1500 WHERE product_id = 'P001';

-- 11. Delete an interaction record:
DELETE FROM Interactions WHERE interaction_id = 5;

-- 12. Retrieve the top 5 users with the highest total purchase amount:
SELECT user_id, SUM(total_amount) AS total_purchase_amount 
FROM Past_Purchases 
GROUP BY user_id 
...
-- 13. Count the number of unique brands in the Products table:
SELECT COUNT(DISTINCT brand) AS unique_brands FROM Products;

-- 14. Window function to rank products by price within each category:
SELECT product_id, product_name, category, price, 
       RANK() OVER (PARTITION BY category ORDER BY price) AS price_rank
FROM Products;

-- 15. Common Table Expression (CTE) to find the average price of products:
WITH AvgPrice AS (
    SELECT AVG(price) AS average_price FROM Products
)
SELECT * FROM Products WHERE price > (SELECT average_price FROM AvgPrice);

-- 16. Create an index on the user_id column of the Past_Purchases table:
CREATE INDEX idx_user_id ON Past_Purchases(user_id);

-- 17. Retrieve the product with the highest total purchase amount:
...
-- 18. Create a view to show interactions with product details:
CREATE VIEW InteractionDetails AS
SELECT i.*, p.product_name, p.category, p.brand 
FROM Interactions i
JOIN Products p ON i.product_id = p.product_id;

-- 19. Rollback a transaction if an error occurs while updating interactions:
START TRANSACTION;
...
-- 20. Count the number of interactions per product
SELECT product_id, COUNT(*) AS interaction_count
FROM Interactions
GROUP BY product_id;

-- 21. List top N most popular products based on interactions
SELECT product_id, COUNT(*) AS interaction_count
FROM Interactions
GROUP BY product_id
...
-- 27. Retrieve interactions for products with prices above the average price:
SELECT *
FROM Interactions
WHERE product_id IN (
    SELECT product_id
...
-- 28. Create a CTE to calculate average product price by category:
...
-- 29. Use a CTE to find the top 3 users with the highest total purchase amounts:
...
-- 30. Calculate the percentage contribution of each product to the total sales amount:
...
-- 31. Identify users who made purchases of more than $500 in a single transaction:
SELECT user_id, purchase_id, total_amount
FROM Past_Purchases
WHERE total_amount > 500;

-- 32. Calculate the average time between consecutive purchases for each user:
...
-- 33. Identify products that have been interacted with but not purchased:
...
-- 34. Find users who made purchases in the first and last quarter of the year:
SELECT user_id
FROM Past_Purchases
WHERE EXTRACT(QUARTER FROM purchase_date) = 1
   OR EXTRACT(QUARTER FROM purchase_date) = 4;

-- 35. Calculate the average quantity of products purchased by users who interacted with products priced above the average price:
...
-- 36. Find users who have interacted with products across multiple categories:
SELECT user_id
FROM (
